...
const Details = ({ target, language}) => {
    ...
    return (
        <StyledDetails>
            <h3>{languages[language].lastChecked}: {t[0]} {t[1]}</h3>
            <h3>{languages[language].currency}: {target}</h3>
        </StyledDetails>
    )
}
...
const Topbar = ({data, onClick}) => {
    ...
    return (
        <Wrapper>
            ...
            {data && <Details target={data.target} language={language} />}
            <Button variant="contained" onClick={onClick}>{languages[language].check}</Button>
        </Wrapper>
    )
}
